...
- Pressing or dragging anywhere in the radial dial quadrant changes zoom without starting a scan session. The protected no-start region is the full quarter-circle area bounded by the arc's outer edge and the adjacent bottom and vertical viewport edges, not only the visible glass band.\r
...
\`\`\`\r
\r
\`interaction\` is a reusable presentation choice, not host policy. \`"unlocked"\` is the default: the live feed and zoom dial remain usable behind the prompt. \`"locked-glass"\` temporarily locks scan and zoom input and shows the light interaction-glass layer over the feed and dial while prompt actions are available. Prompt and radial actions remain above that layer and usable. This option may be applied to any future status or dialog, but the shared example catalog currently uses it only for accepted reference UI states; ordinary dialogs remain unlocked and glass-free by default.\r
\r
...
\r
After deployment propagation, run \`npm run verify:live -- <https-origin>\`. The verifier loads all four accepted published paths in headless Chromium, requires the scanner dialog and radial dial to render without console/page errors, checks the mobile selection guard, and clicks the protected dial arc in every scanner/emulator. It then opens the locked Assign state through each skeleton's development API or emulator control, requires visible interaction glass, activates the scoped popup Confirm action, and requires the status, glass, and radial actions to dismiss. The same gate requires the deprecated \`/qr-scanner/\`, \`/qr-scanner/skeleton/\`, and \`/qr-scanner/emulator/\` paths to return HTTP 404.\r
\r
