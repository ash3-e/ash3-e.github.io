...
\r
interface ScannerPresentationOptions {
  interaction?: "unlocked" | "locked-glass";
  mirrorActionsInPrompt?: boolean;
  dismissAfterAction?: boolean;
  dismissOnScreenTap?: boolean;
}
\`\`\`\r
\r
\`interaction\` is a reusable presentation choice, not host policy. \`"unlocked"\` is the default: the live feed and zoom dial remain usable behind the prompt. \`"locked-glass"\` temporarily locks scan and zoom input and shows the interaction layer over the feed and dial while prompt actions are available. The interaction layer uses the exact clear-status liquid-glass fill and backdrop lens at full material opacity; status, result, dialogue, warning, and camera-menu popups share that fill, lens, and popup shadow, while the viewport overlay deliberately adds no outer shadow. Prompt and radial actions remain above it and usable. A presentation lock affects input only: it never pauses, stops, dims, freezes, or replaces the live camera stream. This option may be applied to any future status or dialog, but the shared example catalog currently uses it only for accepted reference UI states; ordinary dialogs remain unlocked and glass-free by default.

Every visible status becomes dismissible from a non-control screen tap 500 ms after it appears. The guard prevents the gesture that produced a warning from immediately clearing it. A host can give a health-warning dialogue the same behavior with \`dismissOnScreenTap: true\`; confirmation and destructive dialogues remain action-controlled by default. Explicit buttons and the radial slider always take precedence over tap-to-dismiss. Dismissing a status also clears its temporary action presentation and targeting state, but does not alter camera lifecycle or a host-owned input lock.
\r
...
//# sourceMappingURL=emulator-fullscreen-AUgtl427.js.map
